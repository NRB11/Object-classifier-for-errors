...
import time  # For timing classification

# Initialize global variables
csv_filename = "KNN_Classifier_v6/training_data_v6.csv"
...
def classify_new_image():
    ...
    def display_image(idx):
        if idx >= len(image_files):
            ...
            print(f"Total images processed: {len(classification_times)}")
        ...
        classification_time = t2 - t1
        classification_times.append(classification_time)
        print(f"Image {idx+1}/{len(image_files)} - Time: {classification_time:.3f}s - Prediction: {prediction}")
